...
\begin{itemize}
\item \textbf{Goal in Context}: The goal of this requirement
\item \textbf{Preconditions}: The precondition (like username must not already exist)
\item \textbf{Successful End Condition}: What normally happens
\item \textbf{Failed End Condition}: What happens if there is an error (if not possible just say that)
\item \textbf{Actors}: You know this
\item \textbf{Trigger}: Just put something boring here (customers apparently don't get the obvious)
\end{itemize}
