...
The software can be used in any auditorium to organise sale of tickets of various shows. This software consists of the following functions:
\begin{enumerate}
	\item Adding new events as per availability of the Auditorium
...
\begin{enumerate}
	...
	\item User: A user has his personal details like user-name, password etc.
\end{enumerate}


\section{Operating Environment}

We are testing this to work on the following tech stack- JDK-11+, MongoDB 4.4+, Apache Maven 3.6+, and Node 14.5+. We have tested it to run on reasonably recent modern browser like Google Chrome 88+, Firefox 86+ on amd64 architecture.
...
This section describes the API that the backend exposes. The API is listed in Table \ref{table:api}. Note that the endpoint is the tail of the URL after the base address. Eg: If the end-point is \texttt{/users}, and the base URL is \texttt{http://localhost:8080}, then the actual URL for this end-point is \url{http://localhost:8080/users}. Also for an endpoint like \texttt{/users/\{id\}}, the \texttt{id} is a request parameter, passed via the URL, a typical REST convension.
\begin{table}
	\centering
	\caption{SAMS API}
	\label{table:api}
	...
	\makebox[\textwidth][c]{\begin{tabular}{cll}
		...
		POST & \texttt{/expenditures} & Create an expenditure \\
...
The back-end API is to be accessed via the HTTP (Hyper-Text Transfer Protocol). It is to be noted that in the version of SAMS being delivered, TLS (Transport Layer Security) will \emph{not} be used. Passwords are also stored in plain-text. This suffices for the purpose of this software.
...
\item \textbf{Actors}: SM
\item \textbf{Trigger}: In manager dashboard, there is a button for viewing shows.
\end{itemize}
...
\section{Sign-up}
\begin{itemize}
\item \textbf{Goal in Context}: Create an account for a customer
\item \textbf{Preconditions}: A User with the same username must not exist.
\item \textbf{Successful End Condition}: A new customer account with that username is created. 
\item \textbf{Failed End Condition}: If the pre-condition is not met, no user is created.
\item \textbf{Actors}: Customer 
\item \textbf{Trigger}: Customer presses the sign-up button on the login page.
\end{itemize}


\section{View Shows (Customer)}
\begin{itemize}
...
\begin{itemize}
\item \textbf{Goal in Context}: Show tickets booked by a customer.
\item \textbf{Preconditions}: A customer must be logged in.
\item \textbf{Successful End Condition}: Show tickets for selected user. 
\item \textbf{Failed End Condition}: No fail condition.
\item \textbf{Actors}: Customer
...
\item \textbf{Preconditions}: Salesperson must be logged in, there must be tickets available for the show, the show must not yet have started. 
\item \textbf{Successful End Condition}: One ticket is booked for a customer for a show.
\item \textbf{Failed End Condition}: In case of the precondition failing, no ticket is created.
...
\begin{itemize}
	\item The SM cannot be incapacitated, there must always an SM.
	\item No one should tamper with the database.
	\item The API must not be manually tampered with.
	\item The front-end must not be reloaded or closed down mid-operation -- it provides a warning on trying to do so but cannot prevent it.
\end{itemize}
...
\begin{figure}
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{showCrop.png}}
	\caption{Class Diagram for Shows}
	\label{fig:show-cd}
\end{figure}
\begin{figure}
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{ticketCrop.png}}
	\caption{Class Diagram for Tickets}
	\label{fig:ticket-cd}
\end{figure}
\begin{figure}
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{transactionCrop.png}}
	\caption{Class Diagram for Transactions}
	\label{fig:transaction-cd}
\end{figure}
\begin{figure}
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{expenditureCrop.png}}
	\caption{Class Diagram for Expenditures}
	\label{fig:expenditure-cd}
\end{figure}
\begin{figure}
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{userCrop.png}}
